...
async function sendResendEmail(apiKey: string, payload: ResendSendRequest): Promise<void> {
  ...
  // Resend uses 429 for rate-limits, and returns JSON with message details.
  let bodyText = ''
  try {
  ...
  const err = new Error(`Resend send failed: ${res.status} ${res.statusText}${bodyText ? ` - ${bodyText}` : ''}`)
  ...
  if (retryAfter) {
    ...
    if (Number.isFinite(secs)) {
      ;(err as any).retryAfterSeconds = secs
    }
  }
...
Deno.serve(async (req) => {
  ...
  for (const queue of ['auth_emails', 'transactional_emails']) {
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      try {
        // Resend requires a verified "from" identity. Prefer RESEND_FROM if set,
        // otherwise fall back to the queued payload.from.
        const from = (resendFrom && resendFrom.trim()) ? resendFrom.trim() : (payload.from as string)

        // Provide List-Unsubscribe headers when we have a token.
        const headers: Record<string, string> = {}
        const unsubscribeToken = payload.unsubscribe_token as string | undefined
        const projectUrl = Deno.env.get('PUBLIC_SITE_URL') // optional, e.g. https://hr.cauca.io.vn
        if (unsubscribeToken && projectUrl) {
          const baseUrl = projectUrl.endsWith('/') ? projectUrl.slice(0, -1) : projectUrl
          const unsubUrl = `${baseUrl}/unsubscribe?token=${encodeURIComponent(unsubscribeToken)}`
